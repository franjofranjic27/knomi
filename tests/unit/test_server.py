...
def _make_client(collection: str = "test-col") -> tuple[TestClient, MagicMock]:
    """Return a TestClient with a mocked retrieval pipeline."""
    config = Config(store={"collection": collection})
    mock_retriever = MagicMock()
    mock_retriever.search.return_value = _search_results()
    with patch("knomi.serve.server.Retriever", return_value=mock_retriever):
        app = create_app(config)
    return TestClient(app), mock_retriever


def test_health_returns_ok() -> None:
    client, _ = _make_client(collection="my-col")
...
def test_query_calls_retriever_with_query_string() -> None:
    client, mock_retriever = _make_client()
    client.post("/query", json={"query": "Who wrote Hamlet?", "top_k": 3})
    assert mock_retriever.search.call_args.args[0] == "Who wrote Hamlet?"


def test_query_passes_top_k_to_retriever() -> None:
    client, mock_retriever = _make_client()
    client.post("/query", json={"query": "test", "top_k": 7})
    assert mock_retriever.search.call_args.kwargs["top_k"] == 7
